...
export default function PropostasClienteLayout() {
  ...
  const formatarTelefoneWhatsApp = (
    ...
    return (
      <div className='flex flex-col items-center justify-center min-h-[calc(100vh-200px)] text-center p-4 text-destructive'>
        ...
        <Button
          ...
          className='p-0 h-auto underline mt-4'
        >
          Tentar novamente
        </Button>
      </div>
    ...
    <div className='w-full'>
      ...
      {propostas && Object.keys(propostasAgrupadas).length > 0 ? (
        <Accordion type='single' collapsible className='w-full space-y-4'>
          {Object.entries(propostasAgrupadas).map(
            ([projetoTitulo, propostasDoProjeto]: any) => (
              <AccordionItem
                key={projetoTitulo}
                value={projetoTitulo}
                ...
                <AccordionTrigger className='p-4 font-semibold text-lg hover:no-underline'>
                  <div className='flex items-center gap-4'>
                    ...
                    <Badge variant='secondary'>
                      {propostasDoProjeto.length} proposta(s)
                    </Badge>
                  </div>
                </AccordionTrigger>
                <AccordionContent>
                        ...
                        >
                          <div className='md:col-span-1'>
                            <p className='font-medium text-primary'>
                              {proposta.freelancer?.nome || 'Freelancer'}
                            </p>
                            ...
                            <p className='text-sm text-muted-foreground mt-1'>
                              R${' '}
                              {Number(proposta.valorProposto).toLocaleString(
                                'pt-BR',
                                {
